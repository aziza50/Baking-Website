...
function MenuItemCard({ item, variants, index, router }: any) {
  ...
  return (
    ...
    >
      <div
        className={`${isAvailable ? "bg-white" : "bg-gray-200 opacity-50"} p-4 rounded-xl shadow-2xl mb-6 w-full max-w-[300px]`}
      >
        <div className="relative aspect-[4/5] w-full">
          <Image
            src={`${baseUrl}${item.image_url}`}
            alt={item.name}
            fill
            className={`object-cover rounded-lg ${!isAvailable ? "grayscale" : ""}`}
          />
        </div>
      </div>
      <h3
      ...
      <p className={`${josefin.className} text-sm opacity-90 max-w-[250px]`}>
        {item.description}
      </p>
    </motion.div>
  );
